...
$url = new moodle_url('/mod/checkskill/import_outcomes.php', array('id' => $cm->id));
$PAGE->set_url($url);
...
class checkskill_import_outcomes_form extends moodleform {
    function definition() {
        $mform = $this->_form;
        ...
        $mform->addElement('header', 'formheading', get_string('import_outcomes', 'checkskill'));
...
$form = new checkskill_import_outcomes_form();
        ...
        if (is_readable($filename)) {
            ...
            foreach ($filearray as $row) {
                ...
                if (count($outcome) != count($fields_outcomes)) {
                    $errormsg = get_string('error_number_columns_outcomes', 'checkskill', $row);// "Row Outcome has incorrect number of columns in it:<br />$row";
                    $ok = false;
                    break;
                }
                        ...
                        if (!empty($an_outcome)){
                            ...
                            if (!empty($an_outcome->code_referentiel)){
                                if (!empty($an_outcome->code_referentiel) && !$ok_referentiel){
                                    ...
                                    $newitem_ref->checkskill = $checkskill->id;
                                    ...
                                    if (!empty($newitem_ref->displaytext)) { // Don't insert items without any text in them
                                        if (!$DB->insert_record('checkskill_item', $newitem_ref)) {
                                            $ok = false;
