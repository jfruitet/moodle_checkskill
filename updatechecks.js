...
M.mod_checkskill = {
    ...
    init: function(Y, url, sesskey, cmid, updateprogress) {
        ...
        this.updatelist = [];
        var items = YD.getElementsByClassName('checkskillitem');
        for (var i=0; i<items.length; i++) {
            YE.addListener(items[i], 'click', function (e) {
                M.mod_checkskill.check_click(this, e);
            });
        ...
        window.onunload =  function(e) {
            M.mod_checkskill.send_update_batch(true);
        };
    },
    ...
    update_progress_bar: function() {
        ...
        var prall = YD.get('checkskillprogressall');
        var prreq = YD.get('checkskillprogressrequired');

        var allpercent = (this.optionalchecked + this.requiredchecked) * 100.0 / (this.optionalcount + this.requiredcount);
        var inner = YD.getElementsByClassName('checkskill_progress_inner', 'div', prall)[0];
        var inneranim = YD.getElementsByClassName('checkskill_progress_anim', 'div', prall)[0];
        ...
        var disppercent = YD.getElementsByClassName('checkskill_progress_percent', 'span', prall)[0];
        disppercent.innerHTML = '&nbsp;'+allpercent.toFixed(0)+'% ';

        if (prreq) {
            var reqpercent = this.requiredchecked * 100.0 / this.requiredcount;
            inner = YD.getElementsByClassName('checkskill_progress_inner', 'div', prreq)[0];
            inneranim = YD.getElementsByClassName('checkskill_progress_anim', 'div', prreq)[0];
            ...
            disppercent = YD.getElementsByClassName('checkskill_progress_percent', 'span', prreq)[0];
    ...
    update_server: function(itemid, state) {
        ...
        this.updatetimeout = setTimeout(function() {
            M.mod_checkskill.send_update_batch(false);
        }, 500);
        this.show_spinner();
    },
    ...
    show_spinner: function() {
        this.Y.one('#checkskillspinner').setStyle('display', 'block');
    },

    hide_spinner: function() {
        this.Y.one('#checkskillspinner').setStyle('display', 'none');
    }
}
//});
